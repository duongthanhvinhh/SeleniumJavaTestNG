...
//            if (!file.exists()) {
//                Log4jUtils.info("No Folder: " + path);
//                file.mkdir();
//                Log4jUtils.info("Folder created: " + file);
//            }
//
...
//            Log4jUtils.info("Screenshot taken: " + screenName);
//            Log4jUtils.info("Screenshot taken current URL: " + getCurrentUrl());
//        } catch (Exception e) {
...
//            }
//
//            Log4jUtils.info("Driver for Screenshot: " + DriverManager.getDriver());
...
//    }
    ...
    public static boolean verifyTrue(Boolean condition) {
        if (condition) {
            Log4jUtils.info("Verify TRUE: " + condition);
    ...
    public static boolean verifyElementTextEquals(By by, String text, FailureHandling flowControl) {
        ...
        if (result) {
            Log4jUtils.info("Verify text of an element [Equals]: " + result);
        } else {
            Log4jUtils.warn("Verify text of an element [Equals]: " + result);
        }
    ...
    public static boolean verifyElementTextContains(By by, String text, FailureHandling flowControl) {
        ...
        if (result) {
            Log4jUtils.info("Verify text of an element [Contains]: " + result);
        } else {
            Log4jUtils.warn("Verify text of an element [Contains]: " + result);
        }
    ...
    public static ArrayList getValueTableByColumn(int column) {
        ...
        sleep(1);
        Log4jUtils.info("Number of results for column (" + column + "): " + totalRows.size()); //Không thích ghi log thì xóa nhen
        ...
        if (totalRows.size() < 1) {
            ...
            for (int i = 1; i <= totalRows.size(); i++) {
                ...
                Log4jUtils.info("Row " + i + ":" + title.getText()); //Không thích ghi log thì xóa nhen
            }
        }
